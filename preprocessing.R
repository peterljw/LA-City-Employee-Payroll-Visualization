...
df <- read_csv("City_Employee_Payroll.csv")
...
for(year in years) {
  payments[[toString(year)]] <- df %>%
    filter(Year == year) %>%
    select(`Base Pay`, `Overtime Pay`, `Permanent Bonus Pay`, `Longevity Bonus Pay`, 
...
colnames(df.year) <- c("Base Pay", "Overtime Pay", "Permanent Bonus Pay", "Longevity Bonus Pay", 
                       "Temporary Bonus Pay", "Lump Sum Pay", "Other Pay & Adjustments")
df.year <- cbind("Year" = years, df.year)
saveRDS(df.year, "PaymentsByYear.RDS")
...
saveRDS(long.df, "PaymentsByYear-Long.RDS")
...
df.individual <- df.individual %>%
  select(`Year`, `Base Pay`, `Overtime Pay`, `Other Pay (Payroll Explorer)`, `ID`)
colnames(df.individual) <- c("Year", "Base Pay", "Overtime Pay", "Other Pay", "ID")
saveRDS(df.individual, "PaymentsByIndividual.RDS")
...
saveRDS(df.dept, "PaymentsByDept.RDS")
...
saveRDS(df.benefit, "BenefitsByDept.RDS")
